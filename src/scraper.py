...
class WebScraper:
    ...
    def scrape_products(self, url: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for product in products:
                ...
                product['_config_used'] = self.config["name"] if "name" in self.config else "default_config"
        ...
        for container in containers:
            ...
            for field_name, field_config in self.config["fields"].items():
                try:
                    ...
                    print(f"Advertencia: Error extrayendo campo '{field_name}': {str(e)}")
    ...
    def _extract_field_value(self, container: BeautifulSoup, field_config: Dict[str, Any], base_url: str) -> str:
        """
        ...
        for step in field_config.get("path", []):
            ...
            if step_type == "find":
                element = element.find(tag, class_=class_name) if class_name else element.find(tag)
            elif step_type == "find_all":
                elements = element.find_all(tag, class_=class_name) if class_name else element.find_all(tag)
                index = step.get("index", 0)
                element = elements[index] if elements and len(elements) > index else None
    ...
    def _validate_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in required_keys:
            if key not in config:
                raise ValueError(f"Clave requerida '{key}' no encontrada en la configuración del scraper")
        return config
