...
class FileManager:
    ...
    @staticmethod
    def save_to_json(
        data: List[Dict[str, Any]], file_path: Path, indent: int = 2
    ) -> bool:
        """
...
class DataProcessor:
    ...
    def extract_brand_from_title(
        title: str, known_brands: List[str] = None
    ) -> Optional[str]:
        """
    ...
    def generate_product_stats(products: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if "Marca" in all_fields:
            ...
            stats["brands"] = dict(
                sorted(brand_count.items(), key=lambda x: x[1], reverse=True)
            )
...
class ConfigGenerator:
    ...
    def create_base_config() -> Dict[str, Any]:
        ...
        return {
            ...
            "fields": {
                "titulo": {
                    "path": [{"type": "find", "tag": "h3", "class": "product-title"}],
                    "extract": "text",
                },
                "precio": {
                    "path": [{"type": "find", "tag": "span", "class": "price"}],
                    "extract": "text",
                },
                "enlace": {"path": [{"type": "find", "tag": "a"}], "extract": "href"},
            },
            "metadata": {
